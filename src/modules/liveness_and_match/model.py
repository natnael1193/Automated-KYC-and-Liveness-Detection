...
class IDOCR:
    ...
    def _find_id_number(self, text):
        # Example pattern for a standard 9-12 digit ID or alphanumeric passport
        match = re.search(r'\b[A-Z0-9]{8,15}\b', text.upper())
    ...
    def _get_sex(self, text):
        # Look for a standalone M or F near the SESSO label
        match = re.search(r"(?:SESSO|SEX)\s*\n*\s*([MF])", text, re.IGNORECASE)
    ...
    def _get_birth_date(self, text):
        # Look for date after 'DATE OF BIRTH'
        pattern = r"(?:DATE OF BIRTH|NASCITA)\s*\n*\s*(\d{2}\s\d{2}\s\d{4})"
        match = re.search(pattern, text, re.IGNORECASE)
        return match.group(1) if match else "Not Found"

    def _get_expiry_date(self, text):
        # Look for date near 'EXPIRY' or 'SCADENZA'
        pattern = r"(?:EXPIRY|SCADENZA)\s*\n*\s*.*?\s*(\d{2}\s\d{2}\s\d{4})"
        match = re.search(pattern, text, re.IGNORECASE)
        return match.group(1) if match else "Not Found"